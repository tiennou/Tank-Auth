...
class Tank_auth
{
	...
	{
		if ((strlen($username) > 0) && ! $this->ci->users->is_username_available($username))
		{
		...
		$user = $this->ci->users->create_user($data, ! $email_activation);
		if (is_null($user))
		{
			return NULL;
		}
	...
	{
		return ((strlen($username) > 0) && $this->ci->users->is_username_available($username));
	}
	...
	{
		return ((strlen($email) > 0) && $this->ci->users->is_email_available($email));
	}
	...
	function change_email($email)
	{
		...
		{
			// Email unchanged, keep activation key as is
			$data['new_email_key'] = $user->new_email_key;

			return $data;
		}

		$email_available = $this->ci->users->is_email_available($email);
		if ( ! $email_available)
		{
			$this->error = array('email' => 'auth_email_in_use');
			return NULL;
	...
	function activate_user($user_id, $activation_key, $activate_by_email = TRUE)
	{
		...
		if ((strlen($user_id) === 0) OR (strlen($activation_key) === 0))
		{
			return FALSE;
		}
	...
	{
		if ((strlen($user_id) === 0) OR (strlen($new_pass_key) === 0))
		{
			return FALSE;
		}
	...
	{
		if ((strlen($user_id) === 0) OR (strlen($new_pass_key) === 0) OR (strlen($new_password) > 0))
		{
			return NULL;
		}
		...
		$success = $this->ci->users->reset_password($user_id, $hashed_password, $new_pass_key, $this->ci->config->item('forgot_password_expire', 'tank_auth'));
		if ( ! $success)
		{
			return NULL;
		}
	...
	function change_password($old_pass, $new_pass)
	{
		...
		$success = $hasher->CheckPassword($old_pass, $user->password);
		if ( ! $success)
		{
	...
	function set_new_email($new_email, $password)
	{
		...
		{
			// account email is already being changed to the same address
			$data['new_email_key'] = $user->new_email_key;

			return $data;
		}

		$email_available = $this->ci->users->is_email_available($new_email);
		if ( ! $email_available)
		{
			$this->error = array('email' => 'auth_email_in_use');
			return NULL;
	...
	{
		if ((strlen($user_id) === 0) OR (strlen($new_email_key) === 0))
		{
			return FALSE;
		}
	...
	private function create_autologin($user_id)
	{
		...
		$success = $this->ci->user_autologin->set($user_id, md5($key));
		if ( ! $success)
		{
			return FALSE;
		}
	...
	function is_max_login_attempts_exceeded($login)
	{
		...
		$login_max_attempts = $this->ci->config->item('login_max_attempts', 'tank_auth')

		return $this->ci->login_attempts->get_attempts_num($this->ci->input->ip_address(), $login) >= $login_max_attempts;
	}
	...
	{
		if ( ! $this->ci->config->item('login_count_attempts', 'tank_auth') OR $this->is_max_login_attempts_exceeded($login))
		{
			return;
		}
	...
	private function clear_login_attempts($login)
	{
		...
		$login_attempt_expire = $this->ci->config->item('login_attempt_expire', 'tank_auth');

		$this->ci->login_attempts->clear_attempts($this->ci->input->ip_address(), $login, $login_attempt_expire);
	}
